...
| 退出(1)      | q           | 退出NEMU | q                     | | 单步执行 |
si [N]     | 让程序单步执行N条指令后暂停执行，当N没有给出时，缺省为1 | si 10 |
| 打印程序状态 | info SUBCMD | 打印寄存器状态，打印监视点信息 | info r info w |
| 表达式求值   | p EXPR      |
求出表达式EXPR的值，EXPR支持的运算请见调试中的表达式求值小节 | p $eax + 1 | |
扫描内存(2)  | x N EXPR    |
求出表达式EXPR的值，将结果作为起始内存地址，以十六进制形式输出连续的N个4字节 | x
10 $esp             | | 设置监视点   | w EXPR      |
当表达式EXPR的值发生变化时，暂停程序执行                                       |
w\*0x2000             | | 删除监视点   | d N         | 删除序号为N的监视点 | d 2
|
*/
